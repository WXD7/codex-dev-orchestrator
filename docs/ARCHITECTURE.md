...
flowchart TB
    ...
    S --> G[无状态治理 MCP<br/>Question Ledger、契约、路由、学习编译]
    G --> T[可信控制器<br/>人类 attestation、签名账本、实时快照]
    T --> D{研发控制平面}
...
| 一次运行的签名检查点、恢复点、Agent 心跳与阶段指标 | Kandev/Symphony 等外部可信控制器持有的窄 V2.1 ledger |
...
V2.1 另提供一个不进入 MCP 工具面的运行协议库。控制平面用只在本地内存/安全存储中可见的随机令牌签署人类 attestation、Agent 进展和阶段事件链，Agent 只能看到令牌哈希。普通内容哈希只能发现意外损坏，不能证明写入者身份；因此“修改审计内容后重算 SHA-256”无法通过 replay。账本文件由调用方选择位置，本仓库不把它提升为第二任务事实源。
