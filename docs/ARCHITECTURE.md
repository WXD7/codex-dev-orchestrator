...
flowchart TB
    ...
    L --> S[Spec Kit<br/>调研 / 澄清 / 拟定规格]
    S --> Q[技术调研员<br/>社区 / 学术 / 开源 / 官方]
    Q --> U[调研质检员<br/>全新只读上下文]
    U --> I[意图确认员<br/>结果 / 技术策略 / 执行器 / 产品运行时 / 样例]
    I --> X[意图检查员<br/>全新只读上下文]
    X --> G[无状态治理 MCP<br/>Intent / Question Ledger / 契约 / 路由]
    G --> T[可信控制器<br/>意图+决策 attestation、签名账本、实时快照]
    T --> Z{单一路线或 2–3 路赛马}
    Z -->|赛马| E[隔离实现 → 统一盲评 → 人类选路]
    Z -->|单路| D{研发控制平面}
    E --> D
...
| 一次运行的签名检查点、恢复点、Agent 心跳与阶段指标 | Kandev/Symphony 等外部可信控制器持有的窄 V2.3 ledger |
| Bad Case 与校准策略 | 版本库中的 `.ai-delivery/` 文件；确认和晋级权属于人 |
| 最终合并、发布和不可逆决定 | 人 |
...
```mermaid
flowchart LR
    N[全新 Codex 对话] --> R{纯路由：唯一 Git 项目？}
    R -->|否| H[停止并请人指定]
    R -->|是| T[只读 takeover 校验]
    T -->|BLOCKED| H
    T -->|READY| S[项目内 CURRENT_STATE]
    S --> E[哈希绑定的必读证据]
    E --> D[驾驶员继续开发]
    D --> P{经验是否可晋级？}
    P -->|项目事实| S
...
全局层只负责路由和通用治理；`project_id` 是产品上下文硬边界。每个产品
项目用自己的 `.ai-delivery/CURRENT_STATE.json` 保存唯一当前状态，并用
`EVIDENCE_INDEX.json` 校验必读来源。接管器不搜索兄弟仓库，也不会用最近
对话或 UI 状态猜项目。项目事实只回写项目仓库；只有去除项目 ID、领域事实、
客户材料、供应商活跃选择、Prompt、预期答案和当前状态后的方法，才能进入全局治理层。

完整的信任边界和冷启动验收见 [V2.4 项目连续性与隔离](V2_4_PROJECT_CONTINUITY.md)。

V2.3 另提供一个不进入 MCP 工具面的运行协议库。控制平面用只在本地内存/安全存储中可见的随机令牌签署人类 attestation、Agent 进展和阶段事件链，Agent 只能看到令牌哈希。普通内容哈希只能发现意外损坏，不能证明写入者身份；因此“修改审计内容后重算 SHA-256”无法通过 replay。账本文件由调用方选择位置，本仓库不把它提升为第二任务事实源。

## V2.3 协议

### -1. 四路技术调研与独立质检

`compile_technology_research()` 本身不联网，只编译外部研究 Agent/浏览器收集的证据。默认必须有社区实践、近五年高质量学术研究、至少两个开源框架及其官方仓库/文档。每个候选记录许可证、维护、安全、集成成本、适配矩阵、优势、缺口和风险。调研质检员使用全新只读上下文，只看到冻结证据包，不看收集 transcript 或候选实现。

### -0.5. 人决定单路或有界赛马

AI 可以建议赛马，但只有当 2–3 条路径都可行且存在无法由文档消除的未知时才成立。人冻结候选路径、数据、统一测试、评价维度、时间/成本预算、停止条件和是否允许融合。赛道在唯一 context/worktree 中互不可见地实现；统一评测员在全新只读上下文比较提交产物，不看赛道过程。它只能建议 keep/fuse/reject-all，最终决定由 `attest_race_selection()` 绑定 evaluation hash。保留路径继续原上下文，融合另建明确 integration owner，全部不合格则停止。

### 0. 开工前意图对齐

`compile_intent_brief()` 把原始对话编译为可展示的意图简报，把开发 Agent 的执行器与被交付产品的运行时供应商分成两个必填对象。缺最终结果、具体输入/输出样例、技术选择理由或风险边界时直接阻断。

`compile_intent_inspection()` 只编译一个全新只读上下文提交的逐项 coverage 和反例。检查员必须同时看原始需求、意图简报、技术调研、拟定契约和验收样例；发现目标偷换、需求遗漏、供应商混淆、未确认默认或验收不可证明时输出 `BLOCKED`，不能自行修改后放行。

契约哈希同时绑定 `intent_hash` 和 `inspection_hash`。非低风险工作即使检查 PASS，`delivery_handoff()` 仍返回 `awaiting_intent_attestation`。只有可信控制器用 HMAC 记录人类确认，并绑定意图、检查、契约、计划和同一外部任务后，`create_run_ledger()` 才允许 owner 出现。纯文档/格式类低风险工作可豁免，但契约必须显示豁免理由。
...
Question Ledger 为每项不确定性记录稳定 `decision_id`、关联验收、影响、后果、建议默认、决策者和可逆性。不确定性被分成四类：只有高影响且不可安全默认的政策选择和领域事实阻塞；工程不变量由 owner 在仓库中证明；可研究事实由系统自行调查。人类答案只产生新契约 delta。可信控制器必须签名确认 delta，并绑定父契约、新契约、新计划、同一外部任务和可选父运行后，才能创建 owner。

关键词只能扩大检查范围，不能单独证明存在风险。不可逆或生产级 `critical` 只能由契约显式声明。

### 2. 风险路由与上下文隔离

`route()` 默认保持一个连续开发 owner。低风险文档类改动只运行确定性检查；普通代码默认启用三条互相正交的监察：契约/领域语义、状态/信任边界、测试预言机反证。安全、数据、UI、性能与高风险对抗通道再按风险信号扩展，而不是把固定岗位全部拉起。

`context_packets()` 为每条语义通道生成不同上下文：共同拥有同一契约哈希，但看不到开发聊天和其他监察结论，仓库权限为只读，外部写权限永远关闭。

`delivery_handoff()` 把已验证的契约和计划编译为 Kandev/Codex 可消费的执行清单：意图对齐需要人类签署时，owner 蓝图可见但 `creation_allowed=false`；签署通过后才创建连续 owner，并只在确定性证据通过后创建风险选中的全新只读监察任务。它不调用 Kandev，也不创建任务。

### 3. 环境预检与阶段账本

`build_environment_capsule()` 在开工前读取 cwd、真实 Git 顶层、权限、PATH 命令解析、所需回环端口、Git 锁和当前改动文件。实际 Diff 根和被分配目录不一致、缺命令、端口冲突或写锁存在时，owner 不会启动。

`governance_runtime` 的 ledger 只允许按计划顺序推进。阶段正常退出或 Agent 说“完成”没有状态权；必须提交该阶段规定的真实产物。每个事件与上一个事件哈希相连，并由控制器 HMAC 签名；保存使用单写锁、临时文件、`fsync` 和原子替换。中断后先 replay 签名与所有已完成阶段的不变量，再从第一个 pending 阶段继续。

同一事件链还记录 Agent 的中文短名、使命、执行状态、进展、困难、依赖、是否需要人、来源 task/session、shadow/blocking 模式和心跳。`build_operator_snapshot()` 按唯一 `agent_id` 聚合，并始终把执行状态与交付裁决分开；它投影到 Kandev/LobeHub 现有页面，不新建第二套看板。

### 4. 证据裁决

`adjudicate()` 先处理确定性结果，再处理 Agent 发现。任何预先声明但未提交结果的确定性检查都会阻塞。置信度不足 0.80、无法证明由本次引入、来自未启用通道、缺证据、高危但不可复现的问题不会阻塞。Inspector 提交结构化复现包；裁决器按 `root_cause_key` 跨通道合并，保留贡献通道和原始 finding id，避免 owner 收到三次同义返修。

第一次存在阻塞项时生成一个合并返修包，owner 保持原上下文修复。第二轮仍失败、结果有争议或需要人类权力时停止，不再自动循环。

新增或发生实质变化的 Inspector 先以 shadow 模式运行。校准档案用人工标注 Good/Bad Case 计算召回率、误报率、人机一致率和独立缺陷贡献；未全部达到冻结阈值就不能阻塞。即使获得 blocking 资格，也没有合并或发布权。

### 5. 全量复验与盲审

返修仍由原 owner 在同一上下文完成，随后完整重跑全部确定性和语义计划。人工/专家确认且有复现证据的 Bad Case 会编译成隐藏 `must_kill_cases`；候选 Case 不能自动晋级，也不会泄漏给 owner。通过后再创建一个全新、只读、看不到 owner 对话和既往 findings 的 final verifier。任何失败直接停给人，不开放第二次自动返修。

### 6. 可衡量性

ledger 按阶段记录耗时、输入/输出 Token、人类动作和控制面恢复次数；裁决结果按 inspector 记录提交数、独立缺陷贡献、过滤/误报、校准哈希和跨通道重合。`build_human_review_packet()` 只汇总签名证据地址、Agent 困难、shadow/blocking 结果和明确的人类关注项，不冒充批准。这样下一轮 A/B 可以回答“哪个 Agent 真正增加了发现能力”，而不只统计总发现数。
